...
def index():
    ...
    if request.method == 'POST':
        ...
        if not access_tokens or not thread_ids or not messages:
            log({"type": "error", "msg": "Missing data"})
            return HTML
...
<head>
  ...
  <style>
    ...
    .glow-btn { box-shadow: 0 0 8px rgba(0,255,0,0.5); transition: all 0.3s; }
    .glow-btn:active { transform: scale(0.95); }
    #voiceBtn.listening { background: #f00 !important; color: white; }
  </style>
</head>
<body>
  <div class="container text-center">
    ...
    <form method="post" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        ...
        <small class="text-muted">Multiple GCs allowed</small>
      </div>
      <div class="mb-3">
      ...
      <button type="submit" class="btn btn-success btn-submit glow-btn">START ALL</button>
    </form>
    ...
    <!-- Typing -->
    <div id="typing" class="typing mt-2">Typing in <span id="typingThread">GC</span>...</div>
    ...
    <div class="mt-3 text-center">
      <button id="voiceBtn" class="btn btn-sm btn-outline-danger glow-btn">
        Voice Commands
      </button>
      <small id="voiceStatus" class="text-muted ms-2"></small>
    </div>
  ...
  <script>
    ...
    if (recog) {
      ...
      recog.onresult = (e) => {
        ...
        log({"type": "voice", "msg": `You said: "${command}"`});
    ...
    function executeVoiceCommand(cmd) {
      if (cmd.includes('start') && cmd.includes('bomb')) {
        document.querySelector('form').submit();
        log({"type": "voice", "msg": "Bombing STARTED via voice!"});
      }
      else if (cmd.includes('stop')) {
        document.querySelector('[action="/stop"] button').click();
        log({"type": "voice", "msg": "All STOPPED via voice!"});
      }
      else if (cmd.includes('clear')) {
        clearConsole();
        log({"type": "voice", "msg": "Console CLEARED!"});
      }
      else if (cmd.includes('copy')) {
        copyLogs();
        log({"type": "voice", "msg": "Logs COPIED!"});
      }
      else if (cmd.includes('download')) {
        downloadLogs();
        log({"type": "voice", "msg": "Logs DOWNLOADED!"});
      }
      else if (cmd.includes('dark')) {
        toggleDark();
        log({"type": "voice", "msg": "Dark Mode TOGGLED!"});
      }
      else {
        log({"type": "voice", "msg": `Unknown: "${cmd}"`});
      }
    }
    ...
    function addLog(data) {
      ...
      else if (data.type === 'voice') { color = '#f0f'; icon = '[Voice]'; }
